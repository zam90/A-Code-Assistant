...
public class MainForm extends Application
{
	...
	private void initGameStars(AnchorPane root) {
		...
		for (int i = 0; i < mCurretStars.size(); i++) {
			...
			StartEventHandler eventm = new StartEventHandler(starService);
			starFrame.setOnMouseClicked(eventm);
	...
	class StartEventHandler implements EventHandler
	{
		...
		StartEventHandler(StarService ss)
		{
			starS = ss;
		}
		...
		public void handle(Event event)
		{
			...
			if(starS.tobeClearedStars(base,mCurretStars).size() != 0)
			{
				for(Star star : starS.tobeClearedStars(base,mCurretStars))
				{
