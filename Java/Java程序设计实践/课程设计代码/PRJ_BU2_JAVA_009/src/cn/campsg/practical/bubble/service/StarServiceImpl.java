...
public class StarServiceImpl implements StarService {
	...
	public StarList createStars() {
		log.fatal("开始通过createStars函数创建10*10泡泡糖。");
		...
		log.info("10*10泡泡糖创建结果： 完整的泡泡糖集合StarList。");
		log.debug("10*10泡泡糖创建完毕");
		//powered by Jingyu Zhang
		return stars;
	...
	private void lookupByPath(Star base, StarList sList, StarList clearStars) {
		...
		if (col - 1 >= 0) {
			...
			if (star != null && !clearStars.existed(star)) {
				if (star.getType() == type ) {
					log.debug("被点击的泡泡糖：被点击的泡泡糖（"+base.getPosition().getRow()+","+base.getPosition().getColumn()+"）左侧存在同色泡泡糖。");
		...
		if (col + 1 < StarService.MAX_COLUMN_SIZE) {
			...
			if (star != null && !clearStars.existed(star)) {
				if (star.getType() == type) {
					log.debug("被点击的泡泡糖：被点击的泡泡糖（"+base.getPosition().getRow()+","+base.getPosition().getColumn()+"）右侧存在同色泡泡糖。");
					// 被点击泡泡糖与判定泡泡保持一致时，加入列表。
					clearStars.add(StarsUtil.copy(star));
					// 继续按右侧路径步进判断。
					lookupByPath(star, sList, clearStars);
		...
		if (row - 1 >= 0) {
			...
			if (star != null && !clearStars.existed(star)) {
				if (star.getType() == type) {
					log.debug("被点击的泡泡糖：被点击的泡泡糖（"+base.getPosition().getRow()+","+base.getPosition().getColumn()+"）上方存在同色泡泡糖。");
					// 被点击泡泡糖与判定泡泡保持一致时，加入列表。
					clearStars.add(StarsUtil.copy(star));
					// 继续按上方路径步进判断。
					lookupByPath(star, sList, clearStars);
		...
		if (row + 1 < MAX_ROW_SIZE) {
			...
			if (star != null && !clearStars.existed(star)) {
				if (star.getType() == type) {
					log.debug("被点击的泡泡糖：被点击的泡泡糖（"+base.getPosition().getRow()+","+base.getPosition().getColumn()+"）下侧存在同色泡泡糖。");
					// 被点击泡泡糖与判定泡泡保持一致时，加入列表。
					clearStars.add(StarsUtil.copy(star));
					// 继续按下方路径步进判断。
					lookupByPath(star, sList, clearStars);
	...
	public StarList tobeClearedStars(Star base, StarList mCurrent) throws NoClearedStarsException {
		log.debug("开始通过tobeClearedStars函数获取被点击");
		// 用于保存待清除的泡泡糖
		StarList clearStars = new StarList();
		log.info("被点击的泡泡糖为：被点击的泡泡糖（"+base.getPosition().getRow()+","+base.getPosition().getColumn()+"）");
		...
		log.info("待清除的泡泡糖内存列表：通过程序获取的待清除泡泡糖集合StarList");
		log.debug("成功通过tobeClearedStars获取待消除的泡泡糖");
		return clearStars;
