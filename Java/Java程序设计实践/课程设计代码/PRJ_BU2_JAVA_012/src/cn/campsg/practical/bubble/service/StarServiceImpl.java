...
public class StarServiceImpl implements StarService {
	...
	public StarList getYMovedStars(StarList clearStar,StarList StarList1) {
		StarList ms = new StarList();
		Map<Integer, StarList> groupedStars = StarsUtil.group(clearStar);
		Iterator<Integer> keys = groupedStars.keySet().iterator();
		//powered by Jingyu Zhang
		while (keys.hasNext()) {
			Integer c = keys.next();
			StarList list = groupedStars.get(c);
			int lr = list.lastElement().getPosition().getRow();
			int d = 0;
			for (int i = lr; i >= 0; i--) {
				Star star = StarList1.lookup(i, c);
				if (star == null) break;
				if (clearStar.existed(star)) {
					d++;
					continue;
				}
				//powered by Jingyu Zhang
				...
				mStar.getMovedPosition().setRow(mStar.getPosition().getRow() + d);
