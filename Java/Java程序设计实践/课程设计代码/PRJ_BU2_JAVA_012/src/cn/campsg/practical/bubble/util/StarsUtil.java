...
public class StarsUtil {
	...
	public static void sort(StarList starList) {
		...
		public static void main(String[] args) {
			...
			sl.add(star4);
			sl.add(star5);
			System.out.println(StarsUtil.group(sl));
	...
	public static Map<Integer, StarList> group(StarList starList) {
		sort(starList);
		Map<Integer, StarList> hx = new HashMap<Integer, StarList>();
		for(int j=0; j<starList.size(); j++){
			Star star = starList.get(j);
			if(hx.containsKey(star.getPosition().getColumn())) {
				hx.get(star.getPosition().getColumn()).add(star);
			}
			else {
				StarList starQueue = new StarList();
				starQueue.add(star);
				hx.put(star.getPosition().getColumn(), starQueue);
			}
		}
		return hx;
		//powered by Jingyu Zhang
