...
public class StarServiceImpl implements StarService {
	...
	private void lookupByPath(Star base, StarList sList, StarList clearStars) {
		...
		int c = base.getPosition().getColumn();
		int r = base.getPosition().getRow();
		Star star;
		StarType type = base.getType();
			if (c-1>=0) {
				star =(Star)sList.lookup(r, (c-1));
		...
		/******************** PRJ-BU2-JAVA-004 Task3 ********************/
			if (c+1< StarService.MAX_COLUMN_SIZE) {
				star =(Star)sList.lookup(r, (c+1));
				if (star != null && !clearStars.existed(star)) {
					if (star.getType() == type) {
						clearStars.add(StarsUtil.clone(star));
	...
	public StarList tobeClearedStars(Star base, StarList mCurrent) {
		/******************** PRJ-BU2-JAVA-004 Task2 ********************/
			StarList cs = new StarList();
			cs.add(base);
			lookupByPath(base, mCurrent, cs);
			if (cs.size() == 1)
				cs.clear();
			return cs;
			//powered by Jingyu Zhang
