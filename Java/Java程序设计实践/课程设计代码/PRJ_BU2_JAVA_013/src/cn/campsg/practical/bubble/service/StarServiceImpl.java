...
public class StarServiceImpl implements StarService {
	...
	private ArrayList<Integer> getNullColumns(StarList slist)
	{
	   ArrayList<Integer> movest = new ArrayList<>();
	   for(int i = 0;i<MAX_COLUMN_SIZE;i++)
	   {
	      if(slist.lookup(MAX_ROW_SIZE-1,i)==null)
	      {
	    	  movest.add(i);
	      }
	   }
	...
	public StarList getXMovedStars(StarList currentStarList) {
		...
		int d = 0;
		StarList ms = new StarList();
        List<Integer> nullColumns=getNullColumns(currentStarList);
        for (int i =nullColumns.get(0); i < StarService.MAX_COLUMN_SIZE; i++) {
            ...
            //powered by Jingyu Zhang
            for (int j =(StarService.MAX_ROW_SIZE-1); j >= 0; j--) 
            {
                Star s = currentStarList.lookup(j, i);
                if (s == null) break;
                MovedStar mStar = StarsUtil.toMovedStar(s);
                mStar.getMovedPosition().setColumn(mStar.getPosition().getColumn()-d);
